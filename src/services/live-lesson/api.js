...
export async function getLiveLessons({page = 1, limit = 15} = {}) {
    const res = await apiClient.get('mentor/live-lessons', {params: {page, limit}});
    return res.data;
}

export async function getLiveLesson(id) {
    const res = await apiClient.get(`mentor/live-lessons/${id}`);
    return res.data;
}
...
export async function updateLiveLesson({id, ...payload}) {
    const res = await apiClient.patch(`mentor/live-lessons/${id}`, payload);
    return res.data;
}

export async function deleteLiveLesson(id) {
    await apiClient.delete(`mentor/live-lessons/${id}`);
}
...
export async function uploadRecording({assignmentId, title, file}) {
    ...
    const res = await apiClient.post(`mentor/live-lesson-recordings/assignments/${assignmentId}`, form);
    return res.data;
}
