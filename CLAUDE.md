...
**Theme.** `src/theme.css` (imported after Gravity's own stylesheets) overrides Gravity's brand tokens with a green ramp derived from `public/brand.png` — the logo's average green is `#31cf70`, i.e. `hsl(144, 62%, 50%)`. Gravity's stock brand is *yellow*, so its `--g-color-text-brand-contrast` is already dark text, and the same pairing applies here: the logo green measures 2.04:1 against white but 10.28:1 against dark, so buttons keep the real logo colour instead of a darkened stand-in. `--g-color-text-brand` runs darker (`#1d7c43`, 5.23:1 on white) because it has to pass AA as body text, and inverts lighter in the dark theme. Light is the default mode, and `prefers-color-scheme` is deliberately **not** consulted — only an explicit in-app choice switches it.
